...
#define modify_qemu                                             // If you want to remove all the logging codes from the source, undefine this
#define packet_capacity 500000                                 // 500,000
#define my_debug                                                // Show debug messages or not
#define save_path "C:\\users\\sina\\desktop\\qemu\\inst_log_%d.txt"   // Where to save the file of logs (traces)
#define save_assembly_hex_bytes                                 // Save the hex bytes in target file, (not really useful to show them !)
#define save_gp_registers                                       // Save the general-purpose registers (use it among with do_log variable)
                                                                // as it might makes the execution much slower
                                                                // Note : (r/e flags) are counted as general-purpose
#ifdef modify_qemu
/* ================================================================= */
/*                           Structures                              */
/* ================================================================= */
typedef struct MY_PACKET {
    ...
    #ifdef save_gp_registers
    UINT64 regs[16];          // General-Purpose registers (CPU_NB_REGS)
    UINT64 eflags;            /* eflags register. During CPU emulation, CC
                                    flags and DF are set to zero because they are
                                    stored elsewhere */
    #endif

    #ifdef save_assembly_hex_bytes
    uint8_t instruction_bytes[15]; // maximum instruction length in IA-32e
    int instruction_length;        // the amount of bytes saved on instruction_bytes
    #endif
...
BOOL do_log = TRUE;                                 // This variable determines whether the logger and context
                                                    // saver or each element that help gathering and creating
                                                    // Should or shoul not work

BOOL is_buffer_allocated = FALSE;
BOOL is_buffer_allocated_for_printf_buffer = FALSE;

BOOL is_first_time_logging = TRUE;                  // Is first time getting log is used because we remove
                                                    //  the start of the file (element 0) in the first log

PMY_PACKET my_packet_holder = NULL;
void* my_packet_holder_for_printf_buffer = NULL;

int current_buffer_index = 0;                       // Index of buffer which we currently working on
int log_index = 0;                                  // To create a unique index for each log

/* ================================================================= */
/*                           Inline Funcs                            */
/* ================================================================= */

// In order to avoid multiple printfs, instead convert them to one printf string
static inline char* array_to_str(void *buff_to_store, PMY_PACKET packet , int* SizeToRead) {
    int offset = 0;
    void* buffer = buff_to_store;

    // Iterate through each packet row
    for (size_t i = 0; i < packet_capacity; i++)
    {
        // Set format string for offset (PC)
        #ifdef save_assembly_hex_bytes
        offset += sprintf(buffer + offset, "%016llx :", packet[i].pc);
        #else
        offset += sprintf(buffer + offset, "%016llx", packet[i].pc);
        #endif

        #ifdef save_assembly_hex_bytes
        // Iterate through each bytes of PMY_PACKET.instruction_bytes (PMY_PACKET.instruction_length)
        for (size_t j = 0; j < packet[i].instruction_length; j++)
        {
            offset += sprintf(buffer + offset, " %02x", packet[i].instruction_bytes[j]);
        }
        #endif
        ...
        #ifdef save_gp_registers
        // save the r/e flags
        offset += sprintf(buffer + offset, " flags=%016llx", packet[i].eflags);

        // Save the gp registers

        offset += sprintf(buffer + offset, " R%d = %016llx", 0, packet[i].regs[0]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 1, packet[i].regs[1]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 2, packet[i].regs[2]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 3, packet[i].regs[3]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 4, packet[i].regs[4]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 5, packet[i].regs[5]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 6, packet[i].regs[6]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 7, packet[i].regs[7]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 8, packet[i].regs[8]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 9, packet[i].regs[9]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 10, packet[i].regs[10]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 11, packet[i].regs[11]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 12, packet[i].regs[12]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 13, packet[i].regs[13]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 14, packet[i].regs[14]);
        offset += sprintf(buffer + offset, " R%d = %016llx", 15, packet[i].regs[15]);
        #endif
    ...
    // Save the size, so we could later see how much buffer we need to save
    *SizeToRead = offset;

    return buffer;
}

static inline void print_or_save_logs(void) {

    if (!is_buffer_allocated_for_printf_buffer)
    {
        // * 2 is because I want to hold some other things e.g 0x & \n among with spaces 
        // so it's mandatory to get a bigger buffer but it's somehow stupid and wastes a
        // lot buffer, maybe when it comes to a stable state then I have to compute these
        // kinds of seprator and make a precise decision about the size but for now let it
        // be * 100
        my_packet_holder_for_printf_buffer = malloc(packet_capacity * sizeof(MY_PACKET) * 100);
        #ifdef my_debug
        // print the details of allocated buffer for debugging purpose 
        printf("[*] Allocated buffer for my_packet_holder_for_printf_buffer : 0x%llx , size : %lld \n",
            (UINT64) my_packet_holder_for_printf_buffer, packet_capacity * sizeof(MY_PACKET) * 100);
        #endif


        // Set it to true to avoid further allocation of buffer
        is_buffer_allocated_for_printf_buffer = TRUE;
    }

    #ifdef my_debug
    printf("[*] Saving the results of %d instructions started...\n", packet_capacity);
    #endif

    // Btw, we have to zero the packet
    SecureZeroMemory(my_packet_holder_for_printf_buffer, packet_capacity * sizeof(MY_PACKET) * 100);
    
    // Or save the results into a file
    FILE* file;
    char* path = malloc(MAX_PATH);

    // Zeroing the buffer
    SecureZeroMemory(path, MAX_PATH);

    // Create path for saving logs
    sprintf(path, save_path, log_index);
    ...
    if (file) {
        ...
        char* buffer = array_to_str(my_packet_holder_for_printf_buffer, my_packet_holder, &Size);

        // buffer is (+ 18) means that we have to increase the buffer to remove ("0000000000000000 :\n")
        // from the start of the file, we have to reduce this size from the end to avoid saving memory
        // after our target buffer

        #ifdef save_assembly_hex_bytes
        if (is_first_time_logging)
        {
            Size = Size - 19; // with (" :")
            fwrite(buffer + 19, Size, 1, file);
        }
        #else 
        if (is_first_time_logging)
        {
            Size = Size - 17; // without (" :")
            fwrite(buffer + 17, Size, 1, file);
        }
        #endif
        else
        {
        ...
        #ifdef my_debug
        printf("[*] Log saved (size = %d) : %s\n", Size, path);
        #endif  
    }
    else {
        #ifdef my_debug
        printf("[*] There was error in saving file.");
        #endif  
    }
    // We log it once , so set it to 
    is_first_time_logging = FALSE;
    fclose(file);
}

#ifdef save_assembly_hex_bytes
// Saving mem loads for instructions (not movs)

// Data types for memory loads are : uint8_t, int16_t, uint16_t, uint32_t, uint64_t

// This function aims to save the memory loads for instruction bytes
// Note : It's not for mov(s)
static inline void my_log_manager_for_instr_lds_byte(uint8_t instr_mem) {

    if (is_buffer_allocated)
    {
        // Start pointer of my_packet_holder
        void* ptr = &(my_packet_holder[current_buffer_index].instruction_bytes[0]);

        // Save the loaded byte(s)
        memmove(ptr + my_packet_holder[current_buffer_index].instruction_length, &instr_mem, 1);

        // Add one byte to instruction length
        my_packet_holder[current_buffer_index].instruction_length = my_packet_holder[current_buffer_index].instruction_length + 1;
    }
}
static inline void my_log_manager_for_instr_lds_word(int16_t instr_mem) {
    if (is_buffer_allocated)
    {
        // Start pointer of my_packet_holder
        void *ptr = &(my_packet_holder[current_buffer_index].instruction_bytes[0]);
        ...
        // Save the loaded byte(s)
        memmove(ptr + my_packet_holder[current_buffer_index].instruction_length, &instr_mem, 2);

        // Add one byte to instruction length
        my_packet_holder[current_buffer_index].instruction_length = my_packet_holder[current_buffer_index].instruction_length + 2;

    }
}
static inline void my_log_manager_for_instr_lds_uword(uint16_t instr_mem) {
    if (is_buffer_allocated)
    {
        // Start pointer of my_packet_holder
        void *ptr = &(my_packet_holder[current_buffer_index].instruction_bytes[0]);
...
static inline void my_log_manager_for_instr_lds_udword(uint32_t instr_mem) {
    if (is_buffer_allocated)
    {
        // Start pointer of my_packet_holder
        void *ptr = &(my_packet_holder[current_buffer_index].instruction_bytes[0]);
        ...
        // Save the loaded byte(s)
        memmove(ptr + my_packet_holder[current_buffer_index].instruction_length, &instr_mem, 4);

        // Add one byte to instruction length
        my_packet_holder[current_buffer_index].instruction_length = my_packet_holder[current_buffer_index].instruction_length + 4;
    }
}
#ifdef TARGET_X86_64
static inline void my_log_manager_for_instr_lds_uqword(uint64_t instr_mem) {
    ...
    if (is_buffer_allocated)
    {
        ...
        // Save the loaded byte(s)
        memmove(ptr + my_packet_holder[current_buffer_index].instruction_length, &instr_mem, 8);

        // Add one byte to instruction length
        my_packet_holder[current_buffer_index].instruction_length = my_packet_holder[current_buffer_index].instruction_length + 8;
    }
}
#endif

#endif // for save_assembly_hex_bytes

#ifdef save_gp_registers
// This function aims to save the general-purpose registers + r/e flags
static inline void my_log_manager_for_gp_reflags(CPUX86State *env) {

    // Save the eflags
    my_packet_holder[current_buffer_index].eflags = env->eflags;
    
    // We don't use loops here as we don't want our performance wastes 
    // for computing loop index and compare the results

    // Save the gp registers
    my_packet_holder[current_buffer_index].regs[0] = env->regs[0];
    my_packet_holder[current_buffer_index].regs[1] = env->regs[1];
    my_packet_holder[current_buffer_index].regs[2] = env->regs[2];
    my_packet_holder[current_buffer_index].regs[3] = env->regs[3];
    my_packet_holder[current_buffer_index].regs[4] = env->regs[4];
    my_packet_holder[current_buffer_index].regs[5] = env->regs[5];
    my_packet_holder[current_buffer_index].regs[6] = env->regs[6];
    my_packet_holder[current_buffer_index].regs[7] = env->regs[7];

    // save the gp registers (x86_64)
    #ifdef TARGET_X86_64
    my_packet_holder[current_buffer_index].regs[8] = env->regs[8];
    my_packet_holder[current_buffer_index].regs[9] = env->regs[9];
    my_packet_holder[current_buffer_index].regs[10] = env->regs[10];
    my_packet_holder[current_buffer_index].regs[11] = env->regs[11];
    my_packet_holder[current_buffer_index].regs[12] = env->regs[12];
    my_packet_holder[current_buffer_index].regs[13] = env->regs[13];
    my_packet_holder[current_buffer_index].regs[14] = env->regs[14];
    my_packet_holder[current_buffer_index].regs[15] = env->regs[15];
    #endif

}
#endif

// This function aims to create a new row log based on pc and
// change the previously row to point into a new row
#ifdef save_gp_registers
static inline void my_log_manager_for_pc(target_ulong pc, CPUX86State *env) {
#else
static inline void my_log_manager_for_pc(target_ulong pc) {
#endif
    if (!is_buffer_allocated)
    {
        // Allocate Buffer
        my_packet_holder = (PMY_PACKET) malloc(sizeof(MY_PACKET) * packet_capacity);

        #ifdef my_debug
        // print the details of allocated buffer for debugging purpose 
        printf("[*] Allocated buffer for my_packet_holder : 0x%llx , size : %lld \n",
           (UINT64) my_packet_holder_for_printf_buffer, packet_capacity * sizeof(MY_PACKET));
        #endif

        // Zeroing Memory
        SecureZeroMemory(my_packet_holder, sizeof(MY_PACKET) * packet_capacity);
    ...
    // Check for boundary
    if (current_buffer_index > packet_capacity)
    {
        // Try to get a log 
        print_or_save_logs();

        // Restart using buffer
        current_buffer_index = 0;

        // Make the buffer zero again
        SecureZeroMemory(my_packet_holder, sizeof(MY_PACKET) * packet_capacity);
    }

    // Set the PC
    my_packet_holder[current_buffer_index].pc = pc;

    #ifdef save_gp_registers
    // Save the gp registers + r/eflag
    my_log_manager_for_gp_reflags(env);
    #endif
}
...
static inline uint8_t x86_ldub_code(CPUX86State *env, DisasContext *s)
{
    ...
    uint8_t ret = cpu_ldub_code(env, advance_pc(env, s, 1));
            ...
            if (do_log)
            {
              my_log_manager_for_instr_lds_byte(ret & 0xff);
            }
        #endif  
    #endif  
...
static inline int16_t x86_ldsw_code(CPUX86State *env, DisasContext *s)
{
    ...
    int16_t ret = cpu_ldsw_code(env, advance_pc(env, s, 2));
        ...
        if (do_log)
        {
        my_log_manager_for_instr_lds_word(ret & 0xffff);
        }
        #endif  
    #endif  
...
static inline uint16_t x86_lduw_code(CPUX86State *env, DisasContext *s)
{
    ...
    uint16_t ret = cpu_lduw_code(env, advance_pc(env, s, 2));
        ...
        if (do_log)
        {
        my_log_manager_for_instr_lds_uword(ret & 0xffff);
        }
        #endif
    #endif
...
static inline uint32_t x86_ldl_code(CPUX86State *env, DisasContext *s)
{
    ...
    uint32_t ret = cpu_ldl_code(env, advance_pc(env, s, 4));
            ...
            if (do_log)
            {
                my_log_manager_for_instr_lds_udword(ret);
            }
        #endif
    #endif
...
static inline uint64_t x86_ldq_code(CPUX86State *env, DisasContext *s)
{
    ...
    uint64_t ret = cpu_ldq_code(env, advance_pc(env, s, 8));
        ...
        if (do_log)
        {
        my_log_manager_for_instr_lds_uqword(ret);
        }
        #endif
    #endif
...
static void i386_tr_translate_insn(DisasContextBase *dcbase, CPUState *cpu)
{
    ...
    {
    #ifdef save_gp_registers
        CPUX86State *my_env = cpu->env_ptr;
        my_log_manager_for_pc(pc_next, my_env);
    #else
        my_log_manager_for_pc(pc_next);
    #endif
    }
    #endif
    // End Sina
